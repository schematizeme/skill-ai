...
### 3.1 MCP também é o que você CONSOME — e agente hospedado é a terceira forma

Tratar MCP só como "servidor que eu exponho" deixa de fora as duas formas em que ele mais aparece
hoje, e as duas mudam onde ficam a authz e o segredo:

- **Cliente/connector nativo:** o produto (ou o próprio provider) se conecta a servidores MCP de
  **terceiros** — o agente passa a **consumir** tools que você não escreveu. Aqui o piso inverte de
  lado: **o servidor é fonte hostil** (`seguranca-llm.md`: injeção indireta chega pelo *resultado* da
  tool), o inventário do que está habilitado é **decisão com ADR**, e o **menor privilégio** vale
  por conector — não por aplicação.
- **Agente hospedado pelo provider** (o loop e o sandbox rodando fora da sua infra): muda **onde o
  segredo vive** e **quem executa a tool**. O piso da casa continua: **credencial nunca vai no
  prompt nem no contexto**; se a plataforma oferece cofre/variável gerenciada, é por ali; se não
  oferece, a tool sensível fica **do seu lado**, atrás de uma API sua que faz authz — o agente
  hospedado chama a **sua** API, não o seu banco.
- **A pergunta que decide as três formas é a mesma:** *quem resolve identidade, tenant e escopo?* A
  resposta tem de ser **o seu servidor**, sempre — expor por MCP, consumir por conector ou rodar
  hospedado **não relaxa a authz** (§4). O que muda é onde o PEP fica, e isso precisa estar
  desenhado, não presumido.
- **Registre no MAPA** qual forma o produto usa: expõe, consome, ou é hospedado. As três têm
  superfícies de ataque diferentes, e a `schematize-pentest` (`/pentest-ai`) testa a que você tem.
