...
**Versão:** skill `schematize-ai` v0.4.0. Changelog em `CHANGELOG.md`.
...
| `/ai-eval` | monta/roda a suíte de **evals** — com runner e gate de verdade (`scripts/eval-run.mjs` + `scripts/eval-gate.mjs`, exit `0`/`1`/`2`) — antes de dizer "o prompt está bom" |
...
## Artefatos executáveis (a tese central existe como execução)

O piso 7 desta skill é **"sem eval, não está pronto"** — e até 2026-08-21 a skill entregava
**zero scripts**: a tese central só existia como prosa. Agora ela roda:

| Artefato | O que é |
|---|---|
| `assets/schemas/eval-suite.schema.json` | o **dataset dourado** como formato: repetições, limiar de aprovação, métrica-chave, casos com **fatia** e **oráculo** |
| `scripts/eval-run.mjs` | o runner. N amostras por caso, 5 oráculos **determinísticos** (`exato`, `contem`, `regex`, `nega`, `json`), métricas por fatia + p95 + tokens, saída JSON |
| `scripts/eval-gate.mjs` | o veredito: `0` passa · `1` **reprova** · `2` inválido. Falha adversarial é **dura**; regressão é medida **na média e por fatia** |
| `scripts/eval.test.sh` | 19 casos — vazamento de system prompt, injeção indireta, caso 1/3, exceção do adaptador, regressão localizada, suíte que encolheu, suíte sem adversarial |
| `assets/templates/adaptador.exemplo.mjs` | o adaptador **vive no projeto**, não na skill: é ele que conhece o provider e a chave |
| `assets/ci/ai-eval.yml` | o workflow — com as duas decisões explícitas (adaptador real vs. falso; job **required**) |

**A fronteira do runner:** ele não conhece provider, não guarda chave e não escolhe modelo. Isso é o
que permite rodar a suíte offline e trocar de modelo sem tocar na skill — e é o que faz o **teste do
próprio gate** ser possível.
