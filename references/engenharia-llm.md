...
- **Uma porta `LLMProvider`** com o mínimo: `complete(messages, tools?, opts) -> {text|tool_calls, usage}`,
  `embed(texts) -> vectors`. Adaptadores: `AnthropicProvider` (default), e outros por config.
...
- **Config, não código:** modelo, provider, temperatura, teto de tokens, timeout e rota de fallback
  vêm de configuração por ambiente. Trocar `claude-sonnet` por outro SKU, ou rotear "barato pro
...
- **Saída nunca vira ação crua.** JSON do modelo **não** é executado (`eval`/exec), **não** vira
  ...
  dispõe.
- **Determinismo onde dá.** `temperature` baixa para extração/classificação; `max_tokens` teto;
  `seed`/parâmetros reproduzíveis quando o provider oferece — facilita eval e reduz surpresa.
